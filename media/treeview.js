  ...
  class Markmap {
    constructor(svg, opts) {
      ...
      this.handleTextClick = async (e, d) => {
        if (d.data.state.line && d.data.state.file) {
    ...
    async getTagsData(tagName) {
      if (this.staticData && this.staticData[tagName]) {
        return Promise.resolve(this.staticData[tagName]);
      }
      if (vsCodeApi) {
        return vsCodeApi.getTags(tagName);
      }
      return Promise.resolve([]);
    }

    async postFileInfo(postData) {
      if (vsCodeApi) {
        vsCodeApi.postFileInfo(postData);
      }
    }
    getStyleContent() {
      const {
    ...
    static create(svg, diagStr, staticData) {
      const mm = new Markmap(svg, null);
      mm.staticData = staticData;
...
})(this.markmap = {}, d3, (window.vscodeWrapper ? new window.vscodeWrapper.VSCodeAPI() : null));
